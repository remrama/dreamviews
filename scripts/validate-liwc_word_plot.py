...
BAR_ARGS = dict(height=0.8, edgecolor="k", lw=0.5, alpha=1)
ERROR_ARGS = dict(ecolor="k", elinewidth=0.5, capsize=0)
...
TXT_BUFF = 0.05  # space between tip of bar and text
...
fig, (topax, ax) = plt.subplots(nrows=2, figsize=FIG_SIZE, sharex=True, gridspec_kw=GRIDSPEC_ARGS)
...
topax.barh(0, dval, xerr=derr, color=color, error_kw=ERROR_ARGS, **BAR_ARGS)
...
for i, txt in enumerate(labels):
    ...
    txt = txt.replace("*", "\u204e")
    ax.text(xloc, yloc, txt, fontstyle="italic", ha=ha_align, va="center")
...
ax.xaxis.grid(True, which="minor", linewidth=0.3, clip_on=False, color=GRID_COLOR)
...
xloc_txt = 0.02 if dval > 0 else 0.98
