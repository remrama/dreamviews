...
parser.add_argument(
    "--white", action="store_true", help="To ignore labels and just plot all data in white"
)
parser.add_argument(
    "--restrict", action="store_true", help="To restrict data to lucid and non-lucid labels"
)
...
TICK_WIDTH = dict(major=1, minor=0.3)
...
LEGEND_ARGS = {
    ...
    "handletextpad": 0.2,  # space between legend markers and labels
}
...
if not WHITE:
    ...
    ax1_handles = [
        plt.matplotlib.patches.Patch(
            edgecolor="none", facecolor=c.COLORS[cond], label=LEGEND_LABELS[cond]
        )
        for cond in LUCIDITY_ORDER
    ]
    ax1_legend = ax1b.legend(
        handles=ax1_handles, bbox_to_anchor=(0.6, 0.55), loc="lower left", **LEGEND_ARGS
    )
    # Top legend
    ax2_handles = [
        plt.matplotlib.patches.Patch(
...
ax1a.yaxis.set(
    major_locator=plt.MultipleLocator(major_tick_loc_left),
...
counts_txt = rf"$n_{{total}}={n_total_posts}$"
