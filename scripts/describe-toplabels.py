...
for col in ["tags", "categories"]:
    ...
    label_counts = labels[col].value_counts().rename("n_posts").rename_axis(singular)

    # Get a count of how many unique users there are for each label
    user_counts = labels.groupby(col).user_id.nunique().rename("n_users").rename_axis(singular)
    ...
    # export
    export_stem = f"describe-top{col}"
    c.export_table(res, export_stem)
