...
# select few columns to keep in the common user file
TRIMMED_COLUMNS = [
    "join_date",
    "last_activity",
    "gender",
...
import_fname_user_key = os.path.join(c.DATA_DIR, "derivatives", "users-raw2anon_key.json")

export_fname = os.path.join(c.DATA_DIR, "derivatives", "users-attributes.tsv")
export_fname_full = os.path.join(c.DATA_DIR, "derivatives", "users-attributes_full.tsv")
...
with zipfile.ZipFile(import_fname_html, mode="r") as zf:
    ...
    for fn in tqdm.tqdm(fnames[:20], desc="parsing html user pages"):
...
df[TRIMMED_COLUMNS].to_csv(export_fname, **export_parameters)
df.to_csv(export_fname_full, **export_parameters)
