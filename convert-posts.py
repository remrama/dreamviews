...
export_fname_posts   = os.path.join(c.DATA_DIR, "derivatives", "posts-raw.tsv")
export_fname_userkey = os.path.join(c.DATA_DIR, "derivatives", "users-anon_key.json")
...
for html_byt in tqdm.tqdm(html_files, desc="parsing html dream journal pages"):
    ...
    for report, user, date, title in zip(page_reports,
        ...
        split_rule = r"Tags:|Categories" if "Tags:" in report_txt else r"Categories"
        ...
        if "Tags:" in report_txt:
            ...
            tags = None
        dream_txt = dream_txt.strip() # there's always a space on the end, from the re.split Categories string

        # make sure there are SOMEEE characters.
        # this isn't really a length limitation, that will happen in the cleaning script.
        # this is just to make sure there is *something* there
...
df.to_csv(export_fname_posts, sep="\t", encoding="utf-8",
...
with open(export_fname_userkey, "wt", encoding="utf-8") as outfile:
