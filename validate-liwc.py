...
parser.add_argument(
    ...
    "--words",
    action="store_true",
    help="Get individual word contributions. Slower, more memory, extra files.",
)
...
def tokenize4liwc(doc):
    tokens = tknzr.tokenize(doc.lower())  # Lowercase and break into tokens
    tokens = [t for t in tokens if not (len(t) == 1 and not t.isalpha())]  # Remove isolated punc
    return tokens
...
if not GET_WORD_CONTRIBUTIONS:  # Not using this but leaving it to show the much simpler case
    ...
    df = res.apply(pd.Series).fillna(
        0
    )  # Replace empty cells with zero, bc it means they had none of those words
    ...
    def liwc_single_doc(doc):
        """Return, for a single document the total frequency of each LIWC category,
        ...
        for token in tokenized_doc:
            ...
            # E.g., zombie's, zombies, and zombie need to all be zombie* (if that's in a vocab)
            ...
            if (
                current_state != category_counts
            ):  # True if the token is in *any* category, bc counter changed
                if (
                    token in vocab_fulls
                ):  # It's in normal vocab of non-globbed/stemmed words, just update
