...
for i, export_stem_fig in enumerate(export_stem_plots):
    ...
    if i == 2:
        ymax = 0.003
        ylabel = "density"
        for lucidity, series in (
            df.groupby(["user_id", "lucidity"]).wordcount.mean().groupby("lucidity")
        ):
    ...
    if i == 2:
        ax.yaxis.set(
            major_locator=plt.MultipleLocator(ymax),
            major_formatter=plt.FuncFormatter(c.no_leading_zeros),
        )
    else:
        ax.yaxis.set(
            major_locator=plt.MultipleLocator(ymax / 2),
